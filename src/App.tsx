...
const App = () => {
  ...
  const yRange = useTransform(y, [0, 1], [0, 1], { clamp: false });

  y.onChange((v) => {
    if (divRef.current) {
      divRef.current.scrollTop = v;
    }
  });

  const handleWheel = (e: React.WheelEvent) => {
    e.preventDefault();
    y.set(y.get() + e.deltaY * 0.01);
  };

  return (
    <motion.div ref={divRef} onWheel={handleWheel} style={{ y: yRange }}>
      <article>
        <p>
